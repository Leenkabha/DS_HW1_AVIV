...
for infile in dspotify_generated/*.in; do
  ...
  outfile="student_outputs/$base.out"
  expected="dspotify_generated_out/$base.out"
  ...
  # Run Valgrind
  valgrind --leak-check=full --show-leak-kinds=definite --error-exitcode=1 ./main.out < "$infile" > /dev/null 2> "student_outputs/$base.val"
  if [ $? -eq 0 ]; then
    mem_result="✅"
    ((valgrind_pass++))
  else
